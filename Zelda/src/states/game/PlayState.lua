--[[
    GD50
    Legend of Zelda

    Author: Colton Ogden
    cogden@cs50.harvard.edu
]]

PlayState = Class{__includes = BaseState}

function PlayState:init()
    self.player = Player {
        animations = ENTITY_DEFS['player'].animations,
        walkSpeed = ENTITY_DEFS['player'].walkSpeed,
        
        x = VIRTUAL_WIDTH / 2 - 8,
        y = VIRTUAL_HEIGHT / 2 - 11,
        
        width = 16,
        height = 22,

        -- one heart == 2 health
        health = 6,

        -- rendering and collision offset for spaced sprites
        offsetY = 5
    }

    self.dungeon = Dungeon(self.player)
    self.currentRoom = Room(self.player)
    
    self.player.stateMachine = StateMachine {
        ['walk'] = function() return PlayerWalkState(self.player, self.dungeon) end,
        ['idle'] = function() return PlayerIdleState(self.player, self.dungeon ) end,
        ['swing-sword'] = function() return PlayerSwingSwordState(self.player, self.dungeon) end,
        ['liftOrThrow'] = function() return PlayerLiftOrThrowState(self.player, self.dungeon) end,
        ['potWalk'] = function() return PlayerPotWalkingState(self.player, self.dungeon) end,
        ['idlePotCarrying'] = function() return PlayerIdlePotCarryingState(self.player, self.dungeon) end
    }
    self.player:changeState('idle')

    self.pause = false
end

function PlayState:enter(params)

end

function PlayState:update(dt)
    if love.keyboard.wasPressed('escape') then
        love.event.quit()
    end
    -- PAUSE
    if self.pause then
        if love.keyboard.wasPressed('p') then
            self.pause = false
            love.audio.resume()
        else
            return
        end
    elseif love.keyboard.wasPressed('p') then
        self.pause = true
        love.audio.pause()
    end

    self.dungeon:update(dt)
end

function PlayState:render()
    -- render dungeon and all entities separate from hearts GUI
    love.graphics.push()
    self.dungeon:render()
    if self.pause then
        love.graphics.setFont(gFonts['zelda'])
        love.graphics.setColor(255, 255, 255, 255)
        love.graphics.printf('Pause', 2, VIRTUAL_HEIGHT/2 - MAP_RENDER_OFFSET_Y - 30, VIRTUAL_WIDTH, 'center')
    end
    love.graphics.pop()

    -- draw player hearts, top of screen
    local healthLeft = self.player.health
    local heartFrame = 1

    for i = 1, 3 do
        if healthLeft > 1 then
            heartFrame = 5
        elseif healthLeft == 1 then
            heartFrame = 3
        else
            heartFrame = 1
        end

        love.graphics.draw(gTextures['hearts'], gFrames['hearts'][heartFrame],
            (i - 1) * (TILE_SIZE + 1), 2)
        
        healthLeft = healthLeft - 2
    end
end